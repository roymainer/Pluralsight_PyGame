...
class Ball(GameObject):
    ...
    def __init__(self, position, sprite, game):
        ...
        self.__inMotion = 0  # the ball starts not in motion
    ...
    def updatePosition(self):
        ...
        if not self.isInMotion():
            # if the ball is not in motion, reposition it at the center of the pad
            padPosition = self.__game.getPad().getPosition()
            self.setPosition((
                padPosition[0] + (GameConstants.PAD_SIZE[0] / 2) - self.getSize()[0]/2,
        ...
        if newPosition[1] + size[1] >= GameConstants.SCREEN_SIZE[1]:
            ...
            # when hitting the bottom of the screen, don't play sound
