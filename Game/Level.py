...
class Level:
    ...
    def loadRandom(self):
        ...
        maxBricks = int(GameConstants.SCREEN_SIZE[0] / GameConstants.BRICK_SIZE[0])  # calc max bricks
        rows = random.randint(2, 8)  # get random number of brick rows
        amountOfPowerBricks = 0

        for row in range(0, rows):
            for brick in range(0, maxBricks):
                brickType = random.randint(0, 3)  # randomize a brick type
                if brickType == 1 or amountOfPowerBricks >= 2:
            ...
            x = 0  # reset x position for new line
            y += GameConstants.BRICK_SIZE[1]  # move y to new brick line
