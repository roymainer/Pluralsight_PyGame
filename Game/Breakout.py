...
class Breakout:
    ...
    def __init__(self):
        ...
        self.__level = Level(game=self)

        self.__level.loadRandom()  # when the game loads, load a random level
        ...
        self.screen = pygame.display.set_mode(GameConstants.SCREEN_SIZE,
                                              pygame.DOUBLEBUF, 32)
                                              # pygame.DOUBLEBUF | pygame.FULLSCREEN, 32)

        pygame.mouse.set_visible(0)  # set mouse to invisible
    ...
    def start(self):
        while 1:
            self.__clock.tick(GameConstants.FPS)  # 100 FPS

            self.screen.fill((0, 0, 0))  # black out the screen

            currentScene = self.__scenes[self.__currentScene]  # retrieve the current scene
